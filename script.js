 ...
 setTimeout(() => {
    ...
    if(time.getHours() < 12 && time.getHours() >= 0) {
        alert('good morning ' + name + ', Welcome to our site')
    }
    else if(time.getHours() >= 12 && time.getHours()<= 16) {
        alert('good afternoon ' + name + ', Welcome to our site')
    }
    else if(time.getHours() >= 16 && time.getHours()<= 22) {
        alert('good evening ' + name + ', Welcome to our site')
    }
...
function apay(){
    let reference = Math.random()*1000000;
    let d = parseInt(reference);
    alert('rs' + total + ' Paid Succesfully, reference number: QO' + d);
    total = 0;
